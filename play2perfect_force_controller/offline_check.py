...
1. **Exact-replay self-test**: with ``state_source=joint_target``, ``interp=hold``,
   ``latency_steps=0`` (the play2perfect env applies the target at both physics substeps of a
   frame) and the null force law, the middle layer must reconstruct the recorded
...
Usage::
    ...
    python play2perfect_force_controller/offline_check.py \
        --episode outputs/play2perfect/episodes/tight_insertion/<stamp>/ep_0000
    python play2perfect_force_controller/offline_check.py --episode <episode dir> --chunk 12 --horizon 24
"""
...
from play2perfect_force_controller.middle_layer import HybridForceMiddleLayer, Measured  # noqa: E402
...
from play2perfect_force_controller.policy import ChunkedReplayPolicy  # noqa: E402
...
def exact_replay_self_test(episode: ReplayEpisode, chunk: int, horizon: int) -> float:
    ...
    # what the recording commanded at (frame, step): the previous frame's target for the first
    # STALE_TARGET_STEPS steps (0 here: the play2perfect env applies the new target at every
    # substep of the frame), the current frame's target afterwards  [play2perfect]
    expected = np.empty_like(result["action_steps"])
    for f in range(T):
        prev = episode.joint_target[max(f - 1, 0)]
        expected[f, :STALE_TARGET_STEPS] = prev if f > 0 else episode.joint_target[0]
        expected[f, STALE_TARGET_STEPS:] = episode.joint_target[f]
...
def main() -> int:
    ...
    out_dir = args.out_dir or (PROJECT_ROOT / "outputs" / "play2perfect" / "force_controller" / "offline"
    ...
    hand = [i for i, n in enumerate(episode.joint_names) if n not in ARM_JOINT_NAMES]
