...
class ReplayEpisode:
    ...
    def padded(self, num_frames: int) -> "ReplayEpisode":
        """A copy extended to *num_frames* by repeating the last frame (to compare against a
        closed-loop rollout that ran longer than the recording); unchanged if long enough."""
        extra = int(num_frames) - self.num_frames
        if extra <= 0:
            return self
        ...
        return replace(
            self,
            joint_pos=pad(self.joint_pos), joint_vel=pad(self.joint_vel), joint_target=pad(self.joint_target),
            contact_force=pad(self.contact_force), contact_force_steps=pad(self.contact_force_steps),
            contact_object_force_steps=pad(self.contact_object_force_steps),
            contact_point_w=pad(self.contact_point_w),
            object_pos=pad(self.object_pos), object_quat=pad(self.object_quat),
            object_lin_vel=pad(self.object_lin_vel),
            body_pos=pad(self.body_pos), body_quat=pad(self.body_quat),
        )

    def filtered(self, signals: str = "traj", window: int | None = None, polyorder: int | None = None,
                 causal: bool | None = None) -> "ReplayEpisode":
        """A copy with the recorded arrays smoothed (see :mod:`signal_filtering`).

        ``signals`` selects what is filtered: ``"traj"`` the joint trajectory (what a behaviour-
        cloning policy would emit), ``"force"`` the contact channels, ``"all"`` both, ``"none"`` a
        no-op. The defaults come from ``TRAJECTORY_FILTER_SPEC`` / ``FORCE_FILTER_SPEC``; passing
        ``window`` / ``polyorder`` / ``causal`` overrides them for every selected channel.

        Filtering runs in the RECORDED joint order, before :meth:`reordered`, so it is independent
        of the live articulation.
        """
        if signals == "none":
            return self
        from .signal_filtering import FORCE_FILTER_SPEC, TRAJECTORY_FILTER_SPEC, filter_arrays, scaled_spec

        spec: dict = {}
        if signals in ("traj", "all"):
            spec.update(TRAJECTORY_FILTER_SPEC)
        if signals in ("force", "all"):
            spec.update(FORCE_FILTER_SPEC)
        if not spec:
            raise ValueError(f"signals must be traj / force / all / none, got {signals!r}")
        if window is not None or polyorder is not None or causal is not None:
            spec = scaled_spec(spec, window if window is not None else next(iter(spec.values())).window,
                               polyorder, causal)
        data = {k: getattr(self, k) for k in
                ("joint_pos", "joint_vel", "joint_target", "contact_force", "contact_force_steps",
                 "contact_object_force_steps") if hasattr(self, k)}
        out = filter_arrays(data, spec)
        return replace(self, **{k: np.asarray(v, dtype=np.float64) for k, v in out.items()})
