...
class Measured:
    ...
    joint_limits: np.ndarray | None = None      # [J, 2] position limits (robot model), for the
    #                                             at-limit masking of the statics map
...
class ForceFeedbackLaw(ABC):
    ...
    def __init__(self, cfg: ForceLawConfig, joint_names: list[str], fingertip_bodies: list[str],
                 joint_stiffness: np.ndarray | None = None, joint_effort_limits: np.ndarray | None = None):
        self.cfg = cfg
        self.joint_names = list(joint_names)
        self.fingertip_bodies = list(fingertip_bodies)
        self.joint_effort_limits = (
            np.asarray(joint_effort_limits, dtype=np.float64) if joint_effort_limits is not None else None
        )
...
class TaskSpaceForceLaw(ForceFeedbackLaw):
    ...
    **Contact-point channel.** ``e_p = p_ref - p_meas`` is the contact-point tracking error. Its
    ...
    def __init__(self, cfg: ForceLawConfig, joint_names: list[str], fingertip_bodies: list[str],
                 joint_stiffness: np.ndarray | None = None, joint_effort_limits: np.ndarray | None = None,
                 offset_bounds: np.ndarray | None = None):
        super().__init__(cfg, joint_names, fingertip_bodies, joint_stiffness, joint_effort_limits)
        n = len(fingertip_bodies)
        if cfg.map not in ("statics", "constrained"):
            raise ValueError(f"unknown force->offset map {cfg.map!r}")
        # per-joint |offset| bounds of the constrained map: the middle layer's clip, tightened by
        # the effort limit (the PD spring saturates beyond tau_max / K)
        self.offset_bounds = None
        if cfg.map == "constrained":
            bounds = np.full(len(self.joint_names), np.inf) if offset_bounds is None else np.asarray(offset_bounds, dtype=np.float64).copy()
            if self.joint_effort_limits is not None:
                bounds = np.minimum(bounds, self.joint_effort_limits / np.maximum(self.joint_stiffness, 1e-9))
            self.offset_bounds = np.where(np.isfinite(bounds), bounds, 1e3)
        self._prev_ref_mag = None                  # [n] for the point-channel steadiness gate
    ...
    def reset(self) -> None:
        ...
        self._gain[:] = 1.0
        self._prev_ref_mag = None

    def _constrained_offset(self, jac: np.ndarray, f_cmd: np.ndarray, cols: np.ndarray) -> np.ndarray:
        """Bounded least squares per fingertip (``cfg.map == "constrained"``), see config.py.

        With ``tau = K dq`` the force the finger delivers is the row-space part of the torque,
        ``f = -(J J^T)^-1 J tau`` (on-fingertip convention); the rest, ``N tau``, only moves the
        finger. Minimise ``|f(dq) - f_cmd|^2 + |N K dq|^2 / L^2 + reg |dq|^2`` over the joints in
        *cols* subject to ``|dq_j| <= bound_j``. Unbounded, the minimiser is the statics map
        ``-K^-1 J^T f_cmd`` (zero residual, zero null torque); bounded, it is the achievable
        force closest to the command instead of a joint-by-joint clip of the wrong one.
        """
        ...
        if cols.size == 0:
            return dq
        jm = jac[:, cols]                                     # [3, m]
        k = self.joint_stiffness[cols]
        gram = jm @ jm.T + 1e-9 * np.eye(3)
        pinv_t = np.linalg.solve(gram, jm)                    # (J J^T)^-1 J   [3, m]
        a = -pinv_t * k[None, :]                              # f_prod = A dq
        null = (np.eye(cols.size) - jm.T @ pinv_t) * k[None, :]   # N K dq  (torque, N.m)
        inv_arm = 1.0 / max(float(cfg.map_null_arm_m), 1e-4)
        reg = float(cfg.map_reg_n_per_rad)
        m_mat = np.vstack([a, inv_arm * null, reg * np.eye(cols.size)])
        b = np.concatenate([f_cmd, np.zeros(cols.size), np.zeros(cols.size)])
        ub = self.offset_bounds[cols]
        sol = lsq_linear(m_mat, b, bounds=(-ub, ub), method="bvls", tol=1e-10)
        dq[cols] = sol.x
        return dq

    def _limit_mask(self, meas: Measured, push: np.ndarray) -> np.ndarray:
        """Per-joint mask [J] zeroing joints that *push* (sign of the intended offset) into a
        position limit they already sit at: the joint is rigid there and cannot make force."""
        mask = self.joint_mask.copy()
        if meas.joint_limits is None or meas.q is None:
            return mask
        margin = float(self.cfg.limit_margin_rad)
        lo, hi = meas.joint_limits[:, 0], meas.joint_limits[:, 1]
        at_lo = (meas.q <= lo + margin) & (push < 0.0)
        at_hi = (meas.q >= hi - margin) & (push > 0.0)
        mask[at_lo | at_hi] = 0.0
        return mask

    def _point_gate(self, ref: Reference, engaged: np.ndarray, dt: float) -> np.ndarray:
        """Where the point channel may act this step (see ``point_engage_n`` / ``point_steady_rate``)."""
        cfg = self.cfg
        gate = engaged.copy()
        if cfg.point_engage_n > 0.0:
            gate &= ref.f_mag >= cfg.point_engage_n
        if cfg.point_steady_rate > 0.0 and self._prev_ref_mag is not None:
            rate = np.abs(ref.f_mag - self._prev_ref_mag) / max(dt, 1e-9) / np.maximum(ref.f_mag, 1e-6)
            gate &= rate <= cfg.point_steady_rate
        return gate
    ...
    def _point_offset(self, ref: Reference, filt: FilteredForce, meas: Measured,
        ...
        for tip in range(n):
            ...
            # re-solve without the joints this offset would push into a limit they sit at
            mask = self._limit_mask(meas, dq_tip)
            if (mask < self.joint_mask).any():
                jac = meas.tip_jacobians[tip] * mask[None, :]
                dq_tip = cfg.point_kp * (jac.T @ np.linalg.solve(jac @ jac.T + lam2 * eye3, e_p))
    ...
    def compute(self, ref, filt, meas, dt):
        ...
        limit_masked = np.zeros(len(self.joint_names), dtype=bool)
        bound_active = np.zeros(len(self.joint_names), dtype=bool)
        for tip in range(n):
            # dq += -K^-1 J^T f  (the finger pushes on the environment with -f)
            dq_tip = -inv_k * (meas.tip_jacobians[tip].T @ f_cmd[tip])
            mask = self._limit_mask(meas, dq_tip)          # joints pinned at a limit make no force
            limit_masked |= mask < self.joint_mask
            if cfg.map == "constrained" and engaged[tip]:
                cols = np.where(mask > 0.0)[0]
                dq_tip = self._constrained_offset(meas.tip_jacobians[tip], f_cmd[tip], cols)
                bound_active |= np.abs(dq_tip) >= 0.999 * self.offset_bounds
                dq += dq_tip
            else:
                dq += mask * dq_tip

        point_gate = self._point_gate(ref, engaged, dt)
        self._prev_ref_mag = ref.f_mag.copy()
        dq_point, point_err = self._point_offset(ref, filt, meas, point_gate)
        return dq + dq_point, {
            ...
            "dq_point": dq_point,
            "limit_masked": limit_masked,                            # [J] joints masked this step
            "bound_active": bound_active,                            # [J] constrained map at a bound
            "point_gate": point_gate,                                # [n] point channel allowed
        }


def make_force_law(cfg: ForceLawConfig, joint_names: list[str], fingertip_bodies: list[str],
                   joint_stiffness: np.ndarray | None = None, joint_effort_limits: np.ndarray | None = None,
                   offset_bounds: np.ndarray | None = None) -> ForceFeedbackLaw:
    if cfg.law == "null":
        return NullForceLaw(cfg, joint_names, fingertip_bodies, joint_stiffness)
    if cfg.law == "task_space":
        return TaskSpaceForceLaw(cfg, joint_names, fingertip_bodies, joint_stiffness,
                                 joint_effort_limits=joint_effort_limits, offset_bounds=offset_bounds)
    raise ValueError(f"unknown force law {cfg.law!r}; available: ['null', 'task_space']")
...
class HybridForceMiddleLayer:
    ...
    def __init__(self, cfg: ControllerConfig, joint_names: list[str], fingertip_bodies: list[str],
                 ...
                 joint_effort_limits: np.ndarray | None = None):
        self.cfg = cfg
        self.joint_names = list(joint_names)
        self.fingertip_bodies = list(fingertip_bodies)
        self.tracker = ChunkTracker(cfg.reference.interp)
        stiffness = (np.asarray(joint_stiffness, dtype=np.float64) if joint_stiffness is not None
                     else np.asarray(default_joint_stiffness(self.joint_names)))
        ...
        if cfg.force_law.offset_clip_from_effort and joint_effort_limits is not None:
            # [play2perfect] |dq_j| <= tau_max_j / K_j: past that the PD spring is saturated
            self._offset_clip = np.asarray(joint_effort_limits, dtype=np.float64) / stiffness
        if not cfg.force_law.allow_arm_offset:
            for i, name in enumerate(self.joint_names):
                if name in ARM_JOINT_NAMES:
                    self._offset_clip[i] = 0.0
        # the constrained map solves within these same bounds (tightened by the effort limits)
        self.law = make_force_law(cfg.force_law, self.joint_names, self.fingertip_bodies, stiffness,
                                  joint_effort_limits=joint_effort_limits, offset_bounds=self._offset_clip)
    ...
    @property
    def offset_clip(self) -> np.ndarray:
        """[J] per-joint |offset| clamp actually in force (rad)."""
        return self._offset_clip.copy()
